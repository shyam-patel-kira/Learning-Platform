...
resultRouter.get('/reading-answer-result/test/:test_id', (req, res) => {
    ...
    readingAnswerKeyModel.find({
            "test_id": req.params.test_id
        })
        .then(doc => {
            answer_test = doc[0].answers
            answer_test = Object.keys(answer_test).map((key) => [Number(key), answer_test[key]]);

            let user_answer;
            userReadingAnswersModel.find({
                    "test_id": req.params.test_id
                }) //User id is left
                .then(ans => {
                    user_answer = ans[0].answers
                    user_answer = Object.keys(user_answer).map((key) => [Number(key), user_answer[key]]);
                    var i, a, b
                    for (i = 0; i < 40; i++) {
                        a = user_answer[i][1]
                        b = answer_test[i][1]
...
//1. axios.post('/ielts/reading/test1') File:reading.js
