  ...
  render() {
    ...
    for (const i of Object.keys(this.state.passages)) {
      if (i.includes('Passage')) {
        y.push(
          <div key={i} className='passage-content text-customblack'>
            <h2 className='text-3xl my-2 text-customblack'>{i}</h2>
          </div>
        );
        Object.values(this.state.passages[i]).map(z =>
          y.push(
            <div>
              <p className='font-myfonts text-customblack'>{z}</p>
              <br />
            </div>
          )
        );
    ...
    for (let i = 1; i <= 40; i++) {
      text.push(
        <label className='mx-2 px-3 text-right text-customblack'>
          {i + '  '}
          <input
    ...
    if (this.state.loading === true) {
      ...
      return (
        <div className='flex bg-customwhite'>
          <div className='flex-1 w-full border-customblack border-2 mx-2 mt-2 mb-4 bg-gray-100'>
            <h1 className='font-myfonts text-5xl text-customblack my-2 text-center'>
              Test-{this.state.testid}
            </h1>
            <div className='font-myfonts mx-5 text-lg text-customblack'>
              {y}
            </div>
          </div>
          ...
          <div className='w-2/5'>
            ...
            <div className='border-customblack border-2 mx-2 my-2'>
              <h1 className='font-myfonts my-4 mx-2 text-4xl text-customblack'>
